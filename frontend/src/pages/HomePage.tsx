...
import { Upload, FileText, Zap, Brain, Sparkles, Plus, FolderOpen, Clock, Settings } from 'lucide-react';
...
const HomePage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const connectWebSocket = () => {
      ...
      try {
        const BACKEND_WS_URL = 'ws://127.0.0.1:8080';
  ...
  const openDocument = async (documentId: string) => {
    ...
    if (docExists) {
      try {
        ...
        await fetch(`${BACKEND_URL}/api/documents/${documentId}/open`, {
          method: 'POST',
          headers: {
          ...
          <div className="text-center">
            ...
            <div className="flex items-center justify-center gap-4 mb-4">
              ...
              <button
                ...
                title="Test PDF.js Viewer vs Adobe Viewer"
              >
                🧪 Test Viewer
              </button>
        ...
        <div>
          <div className="flex items-center gap-3 mb-8">
            ...
            <div className="relative">
              ...
              {showMenu && (
                <div className="absolute right-0 mt-2 w-36 bg-slate-800 rounded-lg shadow-lg py-2 z-10">
                  <button
                    className="block w-full px-4 py-2 text-sm text-slate-300 hover:bg-slate-700"
                    onClick={() => {
                      setIsSelectMode(true);
                      setShowMenu(false);
                    }}
                  ...
                  <button
                    className="block w-full px-4 py-2 text-sm text-purple-400 hover:bg-slate-700"
                    onClick={() => {
                      // Enable select mode for merging
                      setIsSelectMode(true);
                      setShowMenu(false);
                      console.log('🔗 Merge mode enabled - select documents to merge');
                      // TODO: Add merge-specific UI state
                    }}
                  >
                    Merge
                  </button>
                  <button
                    className="block w-full px-4 py-2 text-sm text-red-400 hover:bg-slate-700"
                    onClick={() => {
